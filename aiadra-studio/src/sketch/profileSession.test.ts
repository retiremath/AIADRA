...
describe('the two entry states (Codex3 B1)', () => {
  it('a create session with nothing drawn has no transaction at all', () => {
    const s = openCreate(PLACEMENT, 'draft1', FRAME, OPTS)
  ...
  it('a drawn create session authors', () => {
    const s = drawLine(openCreate(PLACEMENT, 'draft1', FRAME, OPTS))
  ...
  it('an edit session replaces, and names its feature', () => {
    const s = drawLine(openEdit('feat_0001', BASELINE, FRAME, OPTS), { u: 40, v: 0 }, { u: 60, v: 0.2 })
    const intent = commitIntent(s, 'P-000001')
  ...
  it('an untouched edit re-sends the committed records under their OWN ids', () => {
    const s = openEdit('feat_0001', BASELINE, FRAME, OPTS)
...
describe('drawing', () => {
  it('a two-click line completes itself and re-arms the tool', () => {
    let s = openCreate(PLACEMENT, 'draft1', FRAME, OPTS)
  ...
  it('drawing several shapes accumulates ONE graph', () => {
    let s = drawLine(openCreate(PLACEMENT, 'draft1', FRAME, OPTS))
    s = setTool(s, 'circle')
  ...
  it('a polyline waits for an explicit end', () => {
    let s = setTool(openCreate(PLACEMENT, 'draft1', FRAME, OPTS), 'polyline')
    s = commitPoint(s, { u: 0, v: 0 })
  ...
  it('a zero-size rectangle is dropped rather than sent', () => {
    let s = setTool(openCreate(PLACEMENT, 'draft1', FRAME, OPTS), 'rectangle')
  ...
  it('undo drops the last completed shape only', () => {
    ...
    s = undoLastPart(s)
    expect(s.parts).toHaveLength(1)
    expect(undoLastPart(openCreate(PLACEMENT, 'd', FRAME, OPTS)).parts).toEqual([])
  })

  it('the cursor is live state and never becomes geometry', () => {
    let s = commitPoint(openCreate(PLACEMENT, 'draft1', FRAME, OPTS), { u: 0, v: 0 })
...
describe('the preview is a READ that never ends the session', () => {
  it('the request carries the owner the engine expects', () => {
    const create = previewRequest(drawLine(openCreate(PLACEMENT, 'draft1', FRAME, OPTS)))
    expect(create?.owner).toEqual({ placement: PLACEMENT, candidateKey: 'draft1' })
    const edit = previewRequest(openEdit('feat_0001', BASELINE, FRAME, OPTS))
...
describe('every session payload passes the wire boundary the commit goes through', () => {
  it.each([
    ['a line', drawLine(openCreate(PLACEMENT, 'd', FRAME, OPTS))],
    [
      'a rectangle',
      commitPoint(
        commitPoint(setTool(openCreate(PLACEMENT, 'd', FRAME, OPTS), 'rectangle'), { u: 0, v: 0 }),
        { u: 30, v: 12 },
      ),
    ],
    ['an edited sketch', drawLine(openEdit('feat_0001', BASELINE, FRAME, OPTS), { u: 40, v: 0 }, { u: 60, v: 0.2 })],
  ])('%s', (_label, s) => {
