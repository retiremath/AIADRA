...
7. **Vendored third-party ARTWORK: the FreeCAD icon suite** (added 2026-07-28 per ADR/0045 passes `shell-1`/`sketch-ribbon-1`/`icons-1`) — unmodified LGPL-2.1+ SVG command glyphs vendored from the pinned FreeCAD `1.1.1` tag (per-file embedded `dc:rights` verified; manifest + rules at `aiadra-studio/src/assets/freecad-icons/README.md`; the FreeCAD logo/brand artwork EXCLUDED). Confirm the artifact-level LGPL posture for artwork bundled into the packaged Electron renderer (identifiable, replaceable source-form SVGs) and the required THIRD-PARTY-NOTICES/SBOM entries. A **release-prerequisite confirmation**, not a build blocker.
