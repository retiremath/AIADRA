...
> Last updated: arc 20260519-3 ([ADR/0013](ADR/0013-relationship-type-allocates-to.md)) at 2026-05-19.

## 1. Current Front

**Ring 1 catalogue work — framework complete + six relationship-type ADRs landed (across five decisions). The four-relationship trace family from [ADR/0006 §12](ADR/0006-object-type-requirement.md) is fully schema-pinned.**
...
- Relationship-type ADRs landed: [`satisfies`](ADR/0009-relationship-type-satisfies.md) (trace, Part→Req), [`composed_of`](ADR/0010-relationship-type-composed-of.md) (structural), [`mated_to`](ADR/0011-relationship-type-mated-to.md) (geometric / topological), [`derived_from` and `refines`](ADR/0012-relationship-types-derived-from-and-refines.md) (intra-Requirement trace, combined ADR), [`allocates_to`](ADR/0013-relationship-type-allocates-to.md) (trace, Req→Part/Assembly; first cross-project opt-OUT trace).
...
- New precedent from ADR/0013: **target Type — not relationship shape — governs cross-project endpoint policy** under [ADR/0008 §4](ADR/0008-cross-project-object-identity.md). Trace relationships targeting external Requirements opt in (regulatory clauses); trace relationships targeting external deliverable Objects opt out (route through Binding Objects).
- Three arcs end-to-end on the Claude↔Codex coordination protocol from arc 20260519-1; protocol has caught real Claude misses on round 1 in both ADR arcs (ADR/0006 silent conflict + stale bundle bump in arc-2; cross-project opt-in/opt-out + explicit §C3 clarification in arc-3).

**Next likely arc:** Component per-Type ADR (Track B from snapshot 2026-05-18-12) — first concrete exercise of External pointer Object pattern from [ADR/0008 §3](ADR/0008-cross-project-object-identity.md); unblocks the cross-project `allocates_to` extension deferred by ADR/0013 and the Component target for `composed_of` / `mated_to` deferred by ADR/0010 / ADR/0011; the [Coherence Checklist](#3-coherence-checklist) "AIADRA Core hosts nothing" item will be load-bearing for the first time. Alternative: `parameter_expression` relationship-type ADR (multi-endpoint, indirect-binding, pattern-setting — heavier).
