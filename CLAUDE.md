...
- [SystemState.md](Docs/SystemState.md) — v1 updated through arc 20260531-3 — curated navigation / cache layer; fifteen active patterns; ten Coherence Checklist items; **Spec → code transition COMPLETED; Wedge-001 + Wedge-002 implemented; ADR/0025 production-grade runtime scope pinned; Phase 0 + Phase 1 + Phase 2 F1 SCN ALL landed at [`aiadra-core/`](aiadra-core/)** (Phase 1+2 cumulative ~3,350 LOC new Python; **bundle bumped v0.19.0 → v0.20.0 → v0.21.0**; 61 of 61 tests passing; aiadra version 0.3.0). Phase 2 F1 SCN (arc 20260531-3) extends `parameter_changed` event payload with optional `new_fact_provenance` per [ADR/0025 §4](Docs/ADR/0025-aiadra-core-runtime-scope.md) F1 absorption; BOTH fold codepaths handle wholesale-replace identically per Codex1 B1 (read-side `validation/fold.py` + proposed-state `transaction/boundary.py::_validate_proposed_fold`); per-field CLI flags (`--provenance-category` + `--provenance-agent` + `--provenance-derived-from`) with category-required gate; flat migrator `apply_migration_v0_20_0_to_v0_21_0` (pin-only; idempotent; dry-run); 7 new tests covering schema accept/reject, end-to-end with/without provenance, CLI category gate, migrator behavior. **Two-round Phase 2 arc** (Codex2 signoff) — tightest cadence yet (vs Phase 0's 3 rounds, Phase 1's 7 rounds). §6 Recent Pattern Changes prepends arc 20260531-3 entry; Object Type catalogue still 9, named relationship-type catalogue still 14 (additive SCN; no spec change). **Two remaining SCN arcs**: Phase 3 W3 (per-relationship-type schemas + lookup namespace; v0.21.0 → v0.22.0) → Phase 4 F2 (`threshold_expression` primitive; v0.22.0 → v0.23.0). **Acknowledged minor gaps** (non-blocker per Codex2/Codex5/Codex6/Codex7 verdicts): migrator integration test for v0.19.0 → v0.20.0 (the v0.20.0 → v0.21.0 one exists from Phase 2; v0.19.0 → v0.20.0 still gap); inherited stale-digest migrator CLI handling polish. Next likely arc: Phase 3 W3 SCN.
