...
- [SystemState.md](Docs/SystemState.md) — v1 updated through arc 20260530-1 — curated navigation / cache layer; fifteen active patterns; ten Coherence Checklist items; **Spec → code transition COMPLETED** (Wedge-001 implemented per ADR/0023 in [`spikes/wedge-001/`](spikes/wedge-001/) — 6 CLI commands, 8 JSON Schemas, 12 negative Profile fixtures all rejected, manifest content-hashable, materialized `satisfies` inside Part Revision per ADR/0009 §5); §6 Recent Pattern Changes prepends the first code-producing arc; Object Type catalogue still 9, named relationship-type catalogue still 14 (no spec change this arc — spike is implementation). Next likely arcs: Wedge-002 V&V-instrumented scope ADR (recommended); production-grade `aiadra-core` runtime ADR; Schema Change Notes for three load-bearing FRICTION_LOG.md items.)
