...
- [Glossary.md](Docs/Glossary.md) — v0.23 (new `TestExecution` entry per ADR/0022 — ninth Object Type + first execution-instance Object Type; three new execution-instance relationship-type entries — `executes` / `executed_on` / `produces`; small updates to `EvidenceArtifact` entry noting `collection_context` prose-only role after execution model lands; small update to `tested_against` entry noting multi-hop alternative through TestExecution carries the deliverable-to-evidence linkage by declining-to-add the direct edge; small update to `cites` entry noting TestProcedure-as-source carried forward as deferred per ADR/0022 §10. Continues from v0.22 changes: V&V relationship-type entries from ADR/0021 — `verifies` / `tested_against` / `cites`; first criterion-level addressing landing with per-relationship-schema opt-in discipline; verification-method consistency = tooling-aided diagnostic only; cross-project per-type asymmetry across trace family.)
...
- [SystemState.md](Docs/SystemState.md) — v1 (arc 12 adoption + arc 20260519-2 through 20260520-1 updates — curated navigation / cache layer; fifteen active patterns (new row per ADR/0022: execution-instance relationship category — Fixed-only binding + Revision pinning + cross-project opt-OUT + per-type cardinality-at-release with status-sensitive refinement); ten Coherence Checklist items (new narrow item per ADR/0022: execution-record cardinality invariants); Recent Pattern Changes carries ADR/0015 through ADR/0022; **seed Object Type catalogue at nine (TestExecution added — first execution-instance Object Type)**; **named relationship-type catalogue at fourteen** (executes / executed_on / produces added — first execution-instance relationship category) — operationally complete except `derived_geometry_from`; V&V framework operationally executable end-to-end with execution-instance atomic binding)
